...
<script type="text/javascript">
	...
	function fn_reg(param){
		...
		if(param == "S") {
			if(confirm("등록 하시겠습니까?")){
				$("#num").val(0);
				document.frm.action = "/gnoincoundb/ysmExampleReg.do?save=" +param;
		       	document.frm.submit();
			}
		} else if(param == "U") {
	...
	function fn_appendTr() {
	    ...
	    count ++;
	    $("#survTb > tbody:last").append('<tr><th>첨부파일'+count+'</th>	<td><div class="filebox">	<input class="upload-name" id="upNm1" value="파일선택">	<label class="btn-basic" style="background-color: gray;color:white;" for="file">찾기</label>	<button type="button" class="btn-default" id="addBtn" onClick="javascript:fn_appendTr();" style="background-color:#fc692f;color:white;">추가</button>		<button type="button" class="btn-default" id="addBtn" onClick="javascript:fn_removeTr();" style="background-color:#FF2424;color:white;">제거</button>	<button type="button" id="downBtn" class="btn-basic" style="background-color: green; color: white; height: 29px;" onclick="javascript:fn_down("${detail.fileNm}", "${detail.sysFileNm }", "${detail.filePath }")">다운로드</button>	</div></td></tr>');
	}
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>
		<c:if test="${empty detail.num }">YSM_EXAMPLE 등록</c:if>
		<c:if test="${!empty detail.num }">YSM_EXAMPLE 상세 및 수정</c:if>
	</h2>
			...
			<tr>
				...
				<td>
					<div class="filebox"> 
						...
						<button type="button" class="btn-default" id="addBtn" onClick="javascript:fn_appendTr();" style="background-color:#fc692f;color:white;">추가</button>
					</div>
				</td>
			</tr>
			<tr>
