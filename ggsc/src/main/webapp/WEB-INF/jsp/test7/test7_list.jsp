...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test7RegView.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(ysmNo, num){
		var mnuCd = $("#mnuCd").val();
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test7Dtl.do?mnuCd="+mnuCd+"&ysmNo="+ysmNo+"&num="+num;
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "<c:out value="${pageContext.request.contextPath}" />/test7List.do";
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>노인상담</h2>
		...
		<form name="searchForm" id="searchForm" method="get" onsubmit="return false">
			...
			<div>
				<ul class="tabs">
					<li style="border-style: solid; border-bottom-style:none; border-width: thin; background-color: gray;">노인상담</li>
			...
			<h3 class="h3-title"><i class="fa fa-star"></i>노인상담 - progress</h3>
			<table class="table-style1" style="margin-bottom: 5px;">
				<colgroup> 
					<col width="3%"></col>
					<col width="5%"></col> 
				...
				<tbody id="tby1">
					...
					<c:if test="${detail.size() > 0 }">
						...
						<tr onclick="javascript:fn_detail('${result.ysmNo}', '${result.num}');">
								<td>${result.rnum }</td>
								<td>${result.ysmNo }</td>
			...
			<h3 class="h3-title"><i class="fa fa-star"></i>노인상담 - CLOSE </h3>
			<table class="table-style1" style="margin-bottom: 5px;">
				<colgroup> 
					<col width="3%"></col>
					<col width="5%"></col> 
				...
				<tbody id="tby1">
					<c:if test="${g2detail.size() == 0 }">
						<tr>
							<td colspan="9">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${detail.size() > 0 }">
						<c:forEach items="${g2detail}" var="result">
							<tr>
								<td>${result.rnum }</td>
								<td>${result.ysmNo }</td>
								<td>${result.validation}</td>
			...
			<h3 class="h3-title"><i class="fa fa-star"></i>노인상담 - reoffer </h3>
			<table class="table-style1" style="margin-bottom: 5px;">
				<colgroup> 
					<col width="3%"></col>
					<col width="5%"></col> 
				...
				<tbody id="tby1">
					<c:if test="${g3detail.size() == 0 }">
						<tr>
							<td colspan="9">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${detail.size() > 0 }">
						<c:forEach items="${g3detail}" var="result">
							<tr>
								<td>${result.rnum }</td>
								<td>${result.ysmNo }</td>
								<td>${result.validation}</td>
