...
<script type="text/javascript">
	...
	function fn_reg(){
		...
		document.frm.action = "<c:out value="${pageContext.request.contextPath}" />/test7Reg.do?mnuCd=${mnuCd}";
       	document.frm.submit();
				...
				<tr>
					...
					<td>
						<select id="g2Gb" name="g2Gb">
							...
							<c:forEach items="${g2List }" var="list">
								<option value="${list.odr }" <c:if test="${list.odr == detail.g2Gb }">selected</c:if> /><label for="g2Gb${list.odr}">${list.ysmMclassNm }</label>
							</c:forEach>
						</select>
					</td>
					<th>enormous</th>
					<td>
						<select id="g3Gb" name="g3Gb">
							<option value=''>선택하세요</option>
							<option value='1' <c:if test="${detail.g3Gb == '1' }">selected</c:if>>인위적인1</option>
							<option value='2' <c:if test="${detail.g3Gb == '2' }">selected</c:if>>인위적인2</option>
							<option value='3' <c:if test="${detail.g3Gb == '3' }">selected</c:if>>인위적인3</option>
							<option value='4' <c:if test="${detail.g3Gb == '4' }">selected</c:if>>인위적인4</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<c:forEach items="${g4List}" var="list">
							<input type="radio" id="g4Gb${list.odr}" name="g4Gb" value="${list.odr }" <c:if test="${list.odr == detail.g4Gb}">checked</c:if> /> <label for="g4Gb${list.odr}">${list.ysmMclassNm }</label>
						</c:forEach>
					</td>
					<th>arise</th>
					<td>
						<input type="radio" id="g5Gb1" name="g5Gb" value="1" <c:if test="${detail.g5Gb == '1'}">checked="checked"</c:if> /> <label for="g5Gb1">인위적인라디오1</label>
						<input type="radio" id="g5Gb2" name="g5Gb" value="2" <c:if test="${detail.g5Gb == '2'}">checked="checked"</c:if> /> <label for="g5Gb2">인위적인라디오2</label>
						<input type="radio" id="g5Gb3" name="g5Gb" value="3" <c:if test="${detail.g5Gb == '3'}">checked="checked"</c:if> /> <label for="g5Gb3">인위적인라디오3</label>
						<input type="radio" id="g5Gb4" name="g5Gb" value="4" <c:if test="${detail.g5Gb == '4'}">checked="checked"</c:if> /> <label for="g5Gb4">인위적인라디오4</label>
					</td>
				</tr>
				<tr>
					<th>checkbox</th>
					...
					<th>patent</th>
					<td>
						<input type="checkbox" id="g7Gb1" name="g7Gb" value="1" <c:if test="${fn:contains(detail.g7Gb,1)}">checked</c:if> /><label for="g7Gb1">인위적인체크박스1</label>
						<input type="checkbox" id="g7Gb2" name="g7Gb" value="2" <c:if test="${fn:contains(detail.g7Gb,2)}">checked</c:if> /><label for="g7Gb2">인위적인체크박스2</label>
						<input type="checkbox" id="g7Gb4" name="g7Gb" value="3" <c:if test="${fn:contains(detail.g7Gb,3)}">checked</c:if> /><label for="g7Gb3">인위적인체크박스3</label>
						<input type="checkbox" id="g7Gb5" name="g7Gb" value="4" <c:if test="${fn:contains(detail.g7Gb,4)}">checked</c:if> /><label for="g7Gb4">인위적인체크박스4</label>
					</td>
				</tr>
				<tr>
					<th>radio number</th>
