...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var livgForm = "${result.livgForm}";
		if(livgForm != ""){
			$("#livgForm").val(livgForm).prop("selected", true);
		}
		var houseForm = "${result.houseForm}";
		if(houseForm != ""){
			$("#houseForm").val(houseForm).prop("selected", true);
		}
		...
		name = "liveForm";
		fn_checked(str, name);
		str = "${result.sujtHelcyStat}";
		name = "sujtHelcyStat";
		fn_checked(str, name);
		
		str = "${result.dignMhist}";
		name = "dignMhist";
		fn_checked(str, name);
		...
		str = "${result.econoStat}";
		name = "econoStat";
		fn_checked(str, name);
		
		str = "${result.recntFmlyDeth}";
		name = "recntFmlyDeth";
		fn_checked(str, name);
		
		str = "${result.recntMmanDeth}";
		name = "recntMmanDeth";
		fn_checked(str, name);
		
		str = "${result.olderWorkPtptYn}";
		name = "olderWorkPtptYn";
		fn_checked(str, name);
	...
	<h2 class="h2-title"><i class="fa fa-check-square"></i>개인상담일지 상세</h2>
		<div class="box-style1 x-scroll-auto" >
			<div class="btn" style="float: right; margin: 0;">
				...
				<button type="button" class="btn-basic" onClick="javascript:fn_update('${mnuCd }');" style="background-color: green;color:white;">저장</button>
			</div>	
