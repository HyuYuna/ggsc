...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test13Dtl.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(appleNo){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test13Dtl.do?appleNo=" + appleNo + "&mnuCd=${mnuCd}";
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "<c:out value="${pageContext.request.contextPath}" />/test13List.do";
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>사과</h2>
		...
		<form name="searchForm" id="searchForm" method="get" onsubmit="return false">
			...
			<input type="hidden" id="appleNo" name="appleNo" value="0" />
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<thead>
					<tr>
						...
						<th scope="col">경험유무</th> 
						<th scope="col">회차 select</th> 
						<th scope="col">회차 라디오</th> 
						<th scope="col">첫번째날짜</th> 
						<th scope="col">사례</th> 
						<th scope="col">일시</th> 
						<th scope="col">내용</th> 
					</tr>
				...
				<tbody id="tby1">
					...
					<c:if test="${detail.size() > 0 }">
						<c:forEach items="${detail}" var="result">
							<tr>
								<td onclick="javascript:fn_detail('${result.appleNo}')">${result.rnum }</td>
