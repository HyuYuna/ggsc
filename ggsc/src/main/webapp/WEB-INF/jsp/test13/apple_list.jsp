...
<script type="text/javascript">
	...
	function list(curPage) {
		...
		$.ajax({
			type : "POST",
			url : "<c:out value="${pageContext.request.contextPath}" />/apple_list_ajax.do",
			data : param,
			dataType : "json",
			...
			success : function(json) {
				...
				$.each(json.list, function(i, d) {
					...
					html += '<tr onclick="javascript:fn_reg(\'' + d.addr + '\',\'' + d.epilogue+'\');">';
					html += '<td>' + d.rnum + '</td>';
					html += '<td>' + d.addr + '</td>';
					html += '<td>' + d.epilogue + '</td>';
					html += '</tr>'
				});
	...
	function fn_reg(addr,epilogue) {
		if(opener.document.getElementById("addr")) {
			opener.document.getElementById("addr").value = addr;
		}
		if(opener.document.getElementById("addr2")){
			opener.document.getElementById("addr2").value = addr;
		}
		if(opener.document.getElementById("epilogue")){
			opener.document.getElementById("epilogue").value = epilogue;
		}
		if(opener.document.getElementById("epilogueNm")){
			opener.document.getElementById("epilogueNm").value = epilogue;
		}
		...
		<form name="searchForm" id="searchForm" action="/bbsList.do" method="post" onsubmit="return false">
			...
			<div class="search-box">
				<div class="search-group" style="margin-left:70px;">
					<span class="label"><label>사과 명</label></span>
					<span class="label2">
						<input type="text" name="schEpilogue" id="schEpilogue" value="${vo.schEpilogue }" style="width: 200px;" />
					</span>
				</div>
				<br>
				<div class="btn" style="padding-right:60px;">
