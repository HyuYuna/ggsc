...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test6Dtl.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(num){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test6Dtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "<c:out value="${pageContext.request.contextPath}" />/test6List.do";
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>신드라</h2>
