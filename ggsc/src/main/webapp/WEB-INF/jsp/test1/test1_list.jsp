...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test1Dtl.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(num){
		document.location.href = "<c:out value="${pageContext.request.contextPath}" />/test1Dtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}
	
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "<c:out value="${pageContext.request.contextPath}" />/test1List.do?";
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>FATE</h2>
		...
		<form name="frm" id="frm" method="post" onsubmit="return false">
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<tbody id="tby1">
					<c:if test="${test1List.size() == 0 }">
						<tr>
							<td colspan="5">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${test1List.size() > 0 }">
						<c:forEach items="${test1List}" var="result">
							<tr>
