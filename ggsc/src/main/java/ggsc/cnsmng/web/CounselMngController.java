...
public class CounselMngController {
	...
	public String confirmAjax(CnsAcptVO vo, HttpServletRequest request, ModelMap model) {
		...
		String cnsrId = (String) login.get("userId"); 												// 상담사가 확인을 하는 것 이니 상담사 아이디가 필요함 
		EgovMap Details = counselMngService.getCnsAcceptDtl(String.format("%s", vo.getCaseNo())); 	// 세부 신청 vo 신청자의 정보가 들어가있다
	...
	public String preExamReg(PreExamVO vo, HttpServletRequest request, ModelMap model) {
		...
		if (vo.getExamDocCd().equals("1")) {
			...
			// 상담동의서 등록
			url = "redirect:/privacyAgreement.do?mnuCd=" + mnuCd +"&caseNo=" + caseNo;
		} else if (vo.getExamDocCd().equals("3")) {
			url = "redirect:/scScreeningScale.do?mnuCd=" + mnuCd +"&caseNo=" + caseNo;
		} else if (vo.getExamDocCd().equals("4")) {
			url = "redirect:/pretestList.do?mnuCd=" + mnuCd +"&caseNo=" + caseNo;
		}
		return url;
	}
	...
	public String cnsAgreement(HttpServletRequest request, ModelMap model) {
		...
		if(caseNo !=""){
			...
			cnsAgreDoc = counselMngService.getCnsAgreDocDtl(caseNum); // 동의서를 작성 안 했을 시에 상세조회 에서 값이 나오지 않는다 . 다른 path 에서도 각기 테이블에서 이런 방식으로 체크 할 듯 
		}
	...
	public String finUserPopup(HttpServletRequest request, ModelMap model, UserInfoVO vo) {
		...
		if (userNm.length() > 0) {
			...
			} catch (UnsupportedEncodingException  e) { // 인코딩 오류시 발생하는 ...
	...
	public String cnsAcptRegAjax_test(CnsAcptVO vo, HttpServletRequest request, ModelMap model) {
		...
		System.out.println(login.get("userId").toString());
	...
	@RequestMapping(value = "/confirm_ajax_test.do", method = RequestMethod.POST)
	public String confirmAjax_test(CnsAcptVO vo, HttpServletRequest request, ModelMap model) {
		EgovMap login = (EgovMap) request.getSession().getAttribute("LoginVO");

		String cnsrId = (String) login.get("userId"); 												// 상담사가 확인을 하는 것 이니 상담사 아이디가 필요함 
		EgovMap Details = counselMngService.getCnsAcceptDtl_TEST(String.format("%s", vo.getCaseNo())); 	// 세부 신청 vo 신청자의 정보가 들어가있다
		vo.setZoneGb(Details.get("zoneGb").toString());
		vo.setLocalGb(Details.get("localGb").toString());
		vo.setCenterGb(Details.get("centerGb").toString());
		vo.setSigunCd(Details.get("sigunCd").toString());
		...
		if(vo.getCaseNo() == 0) { // caseNo 라는 고유값이 존재하지 않을 때 실행되는 if
			//counselMngService.updateCnsAcceptCaseNo(vo);
		}
		
		counselMngService.updateConfirm_TEST(vo, cnsrId);
	...
	@RequestMapping(value = "/pretestList_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String pretestList_TEST(PreExamVO vo, HttpServletRequest request, ModelMap model) {
		...
		List<EgovMap> cnsCenterList = adminManageService.getCenterManageList(centerVO);
		model.addAttribute("cnsGbList", cnsGbList);			// 상담구분 
		model.addAttribute("cnsCenterList", cnsCenterList);	// 센터명 구분 
		...
		return "cnsmng/pretestList_test.main";
	}
	
	@RequestMapping(value = "/exiCnsAcceptList_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String exiCnsAcceptList_test(CnsAcptVO vo, HttpServletRequest request, ModelMap model) {
		// 권한 관리 시작
		EgovMap loginVo = (EgovMap) request.getSession().getAttribute("LoginVO");
		...
		List<EgovMap> exiAcceptList = counselMngService.getExiCnsAcceptList_TEST(vo); // 기존접수 관련 
		...
		int totalPageCnt = counselMngService.getExiCnsAcptListTotCnt_TEST(vo);
		model.addAttribute("totalPageCnt", totalPageCnt);
		paginationInfo.setTotalRecordCount(totalPageCnt); // 전체 게시물 건 수
		model.addAttribute("paginationInfo", paginationInfo);
		model.addAttribute("exiAcceptList", exiAcceptList);
		...
		model.addAttribute("vo", vo);
		model.addAttribute("authCd", userAuth);
		return "cnsmng/exiCnsAcceptList_test.main";
	}
	
	@RequestMapping(value = "/cnsAgreement_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String cnsAgreement_test(HttpServletRequest request, ModelMap model) {
		...
		if(caseNo !=""){
			int caseNum = Integer.parseInt(caseNo);
			cnsAgreDoc = counselMngService.getCnsAgreDocDtl_test(caseNum);
		}
		
		model.addAttribute("detail", cnsAgreDoc);

		return "cnsmng/cnsAgreement_test.main";
	}
	
	@RequestMapping(value = "/findUserPopup_test.do", method = { RequestMethod.POST, RequestMethod.GET })
	public String finUserPopup_test(HttpServletRequest request, ModelMap model, UserInfoVO vo) {
		...
		return "common/findUserPopup_test";
	}
	
	@RequestMapping(value = "/cnsAcceptDtl_ajax_test.do", method = RequestMethod.POST)
	public String cnsAcceptDtl_ajax_test(CnsAcptVO vo, HttpServletRequest request, ModelMap model) {
		...
		EgovMap Details = counselMngService.getCnsAcceptDtl_TEST(caseNo);
		model.addAttribute("Detail", Details);
		return "jsonView";
	}
	
	@RequestMapping(value = "/privacyAgreement_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String privacyAgreement_test(HttpServletRequest request, ModelMap model) {
		String mnuCd = request.getParameter("mnuCd") == null ? "" : request.getParameter("mnuCd");
		model.addAttribute("mnuCd", mnuCd);
		String caseNo = request.getParameter("caseNo") == null ? "" : request.getParameter("caseNo");
		model.addAttribute("caseNo", caseNo);
		...
		if(caseNo !=""){
			int caseNum = Integer.parseInt(caseNo);
			privacyDtl = counselMngService.getPrivacyDtl_test(caseNum);
		}
		model.addAttribute("detail",privacyDtl);
		
		return "cnsmng/privacyAgreement_test.main";
	}
	
	@RequestMapping(value = "/scScreeningScale_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String scScreeningScale_test(HttpServletRequest request, ModelMap model) {
		String mnuCd = request.getParameter("mnuCd") == null ? "" : request.getParameter("mnuCd");
		model.addAttribute("mnuCd", mnuCd);
		String caseNo = request.getParameter("caseNo") == null ? "" : request.getParameter("caseNo");
		model.addAttribute("caseNo", caseNo);
		...
		if(caseNo !=""){
			int caseNum = Integer.parseInt(caseNo);
			scScreen = counselMngService.getScScreenDtl_test(caseNum);
		}
		
		model.addAttribute("detail",scScreen);
		
		return "cnsmng/scScreeningScale_test.main";
	}
	...
	public String preExamReg_test(PreExamVO vo, HttpServletRequest request, ModelMap model) {
		...
		if (vo.getExamDocCd().equals("2")) {
			// 상담동의서 등록
			url = "redirect:/privacyAgreement_test.do?mnuCd=" + mnuCd +"&caseNo=" + caseNo;
		} else if (vo.getExamDocCd().equals("3")) {
			url = "redirect:/scScreeningScale_test.do?mnuCd=" + mnuCd +"&caseNo=" + caseNo;
		} else if (vo.getExamDocCd().equals("4")) {
	...
	// 개인상담일지 등록 중복등록 체크
		@RequestMapping(value = "/perCnsRegCheckAjax_test.do", method = { RequestMethod.GET, RequestMethod.POST })
		public String perCnsRegCheckAjax_test(HttpServletRequest request, ModelMap model) {

			String caseNoStr = request.getParameter("caseNo");
			int caseNo = Integer.parseInt(caseNoStr);
			int result = counselMngService.getPerCnsRegCheck_test(caseNo);

			model.addAttribute("result", result);
			return "jsonView";
		}
		
	@RequestMapping(value = "/perCnsReg_test.do", method = RequestMethod.POST)
		public String perCnsReg_test(PerCnsVO vo, HttpServletRequest request, ModelMap model) {

			EgovMap loginVO = (EgovMap) request.getSession().getAttribute("LoginVO");
			vo.setCnsrId(loginVO.get("userId").toString()); // 로그인시 Session 영역에 올라간 데이터 
			...
			GroupVO param = new GroupVO();
			param.setHclassCd("G14");
			List<EgovMap> cnsMethdList = adminManageService.getGroupMngDtlMList(param); // 상담 타입 
			
			vo.setCnsMethdNm((String) cnsMethdList.get(Integer.parseInt(vo.getCnsMethd()) - 1).get("mclassNm"));
			
			counselMngService.insertPerCns_test(vo);

			return "redirect:/perCnsList_test.do?mnuCd=" + mnuCd;
		}
	
	@RequestMapping(value = "/gCnsList_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String gCnsList_test(GcnsVO vo, HttpServletRequest request, ModelMap model) {
		...
		String centerGb = (String) map.get("centerGb");
		String zoneGb = (String) map.get("zoneGb");
		String schCnsGb = vo.getSchCnsGb();			// 권역 
		String schCenterGb = vo.getSchCenterGb();	// 센터 
		...
		return "cnsmng/gCns_list_test.main";
	}
	
	@RequestMapping(value = "/gCnsRegView_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String gCnsRegView_test(GcnsVO vo, HttpServletRequest request, ModelMap model) {
		String mnuCd = request.getParameter("mnuCd") == null ? "" : request.getParameter("mnuCd");
		model.addAttribute("mnuCd", mnuCd);
		...
		return "cnsmng/gCnsRegView_test.main";
	}
	
	@RequestMapping(value = "/gCnsReg_test.do", method = RequestMethod.POST)
	public String gCnsReg_test(GcnsVO vo, HttpServletRequest request, ModelMap model) throws Exception {
		String mnuCd = request.getParameter("mnuCd") == null ? "" : request.getParameter("mnuCd");
		model.addAttribute("mnuCd", mnuCd);

		EgovMap map = (EgovMap) request.getSession().getAttribute("LoginVO");
		vo.setRegId((String) map.get("userId"));  // 등록자 아아디 
		vo.setCnsrId((String) map.get("userId")); // 상담자 아이디 
		
		System.out.println("insertGcns_test facade check");
		counselMngService.insertGcns_test(vo); // detail 테이블에 insert 된다 

		return "redirect:/gCnsList_test.do?mnuCd=" + mnuCd;
	}
	
	@RequestMapping(value = "/userSelPopup_test.do", method = { RequestMethod.POST, RequestMethod.GET })
	public String finUserPopup2_test(HttpServletRequest request, ModelMap model, UserInfoVO vo) {
		
		return "common/userSelPopup_test.main";
	}
	
	@RequestMapping(value = "/userSelPopupAjax_test.do", method = { RequestMethod.GET, RequestMethod.POST })
	public String userSelPopupAjax_test(HttpServletRequest request, ModelMap model, UserInfoVO vo) {
